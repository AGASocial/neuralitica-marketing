...
import {
  MUSETALK_ALLOWED_OUTPUT_HOSTS,
  MUSETALK_DEFAULT_PREDICTION_INPUT,
  MUSETALK_ENV_KEY_NAME,
  MUSETALK_FETCH_MAX_BYTES,
  MUSETALK_FETCH_MAX_REDIRECTS,
  MUSETALK_FETCH_TIMEOUT_MS,
  MUSETALK_REPLICATE_INPUT_FIELDS,
  MUSETALK_REPLICATE_MODEL_VERSION,
...
export type CreateMusetalkLowAdapterParams = {
...
function getReplicateApiToken(): string {
  const token = process.env[MUSETALK_ENV_KEY_NAME];
...
function validateCreateJobInput(input: CreateVideoJobInput): void {
  ...
  if (input.portraitAssetId || input.referenceImageAssetId) {
    throw new ProviderAdapterError(
      "INVALID_PROVIDER_INPUT",
      "MuseTalk does not accept portrait or reference image assets",
    );
  }
  ...
  if (!input.referenceVideoAssetId) {
    throw new ProviderAdapterError(
      "INVALID_PROVIDER_INPUT",
      "Reference loop video asset is required for MuseTalk",
    );
  }
  ...
  if (!input.voiceoverAssetId) {
    throw new ProviderAdapterError(
      "INVALID_PROVIDER_INPUT",
      "Voiceover asset is required for MuseTalk",
    );
  }
}
...
async function downloadProviderOutput(
  ...
  while (true) {
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      redirects += 1;
      if (redirects > MUSETALK_FETCH_MAX_REDIRECTS) {
        throw new ProviderAdapterError(
          "PROVIDER_DOWNLOAD_FAILED",
    ...
    if (contentLength) {
      ...
      if (Number.isFinite(size) && size > MUSETALK_FETCH_MAX_BYTES) {
        throw new ProviderAdapterError(
          "PROVIDER_DOWNLOAD_FAILED",
    ...
    while (true) {
      ...
      if (totalBytes > MUSETALK_FETCH_MAX_BYTES) {
        throw new ProviderAdapterError(
          "PROVIDER_DOWNLOAD_FAILED",
          "Provider output exceeds maximum size",
        );
...
export function createMusetalkLowAdapter(
  params: CreateMusetalkLowAdapterParams,
  ...
  const resolveAssetUrl =
    resolveMediaAssetUrl ??
    (async (assetId: string, clientId: string, kind: "video" | "audio") =>
  ...
  return {
    ...
    async createJob(input: CreateVideoJobInput) {
      ...
      const [referenceVideoUrl, voiceoverAudioUrl] = await Promise.all([
        resolveAssetUrl(
      ...
      const response = await replicateRequest(fetchImpl, token, "/v1/predictions", {
        ...
        body: JSON.stringify({
          ...
          input: {
            [MUSETALK_REPLICATE_INPUT_FIELDS.video]: referenceVideoUrl,
            [MUSETALK_REPLICATE_INPUT_FIELDS.audio]: voiceoverAudioUrl,
            [MUSETALK_REPLICATE_INPUT_FIELDS.bboxShift]:
              MUSETALK_DEFAULT_PREDICTION_INPUT.bbox_shift,
            [MUSETALK_REPLICATE_INPUT_FIELDS.cycle]:
              MUSETALK_DEFAULT_PREDICTION_INPUT.cycle,
          },
        }),
      });
    ...
    async fetchAsset(
      ...
      validateProviderOutputUrl(outputUrl, MUSETALK_ALLOWED_OUTPUT_HOSTS);
