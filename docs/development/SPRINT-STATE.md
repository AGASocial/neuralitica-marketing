...
- 2026-08-31 · US-15.1 Phase B QA re-review `4da9761`: H1/M1/M2 re-confirmed closed. Escalated the validator's new finding from Medium to **High** — after H1's pause fires mid-chain, `resumeWeeklyCycleRun` only acts on `"failed"` step_runs, never calls `advanceWeeklyCycleSlot` for the paused slot's next step; on resume, `reconcileWeeklyCycleRun` immediately resolves back to `partial_failed`/`failed` with no pending rows, and a second resume is refused (`RUN_NOT_RESUMABLE`) — that client's Reel for that slot is never produced, no automated recovery, and `deriveSlotDto` shows it as healthy `"processing"` with zero Operator signal. Breaks the kill switch's core purpose as a safe stop/resume control (SECURITY.md:281). → BUILD/FIX re-delegated (High blocks per gate rules, despite the agent's own "APPROVE WITH CONDITIONS" label).
- 2026-08-31 · US-15.1 Phase B POST-QA-FIX VALIDATE PASS WITH NOTES `a249f3a`: H1/M1/M2 all independently re-verified closed (201/201 tests run directly; 0 new tsc errors vs disposable pre-fix worktree diff). New Medium finding (not prior QA scope): after kill-switch pause, the completed-but-mid-chain step never advances to its next step on resume — pipeline stays orphaned past that slot; not a security/double-spend issue (doesn't reopen H1), conflicts with SECURITY.md:281 "advances only the next eligible step(s)". → QA re-review to call severity/blocking.
- 2026-08-31 · US-15.1 Phase B QA-FIX `60de5f6`: H1 closed — new `pause-weekly-cycle-run-cas.ts` (`running→paused` CAS); `resume-weekly-cycle-from-job.ts` now persists a completed job's step as `completed` (never a synthetic `failed`) and pauses the run instead of discarding paid work; resume retry filter now checks `RETRYABLE_WEEKLY_CYCLE_ERROR_CODES` before re-dispatch. M1 (`.select().maybeSingle()` on CAS guards) + M2 (`claimed_at` staleness field) also fixed. 201/201 tests (up from 191), 0 new tsc errors, eslint clean. → focused re-VALIDATE then QA re-review.
- 2026-08-31 · US-15.1 Phase B QA APPROVE WITH CONDITIONS `cb218ec` (0 Critical, 1 High, 2 Medium, 1 Low): **H1 blocking** — CONTRACT's `running→paused` kill-switch transition never implemented; `resumeWeeklyCycleFromJob` marks a step `failed`/`LIVE_DISABLED` even when the underlying job completed successfully, discarding paid work and enabling double-spend on Operator resume (test suite codifies the discard at `resume-weekly-cycle-from-job.test.ts:180-195`). M1/M2 non-blocking CAS/outbox hygiene (contained by other guards). Budget/consent, IDOR, no-publish boundary, FE/DTO leakage all clean. → BUILD/FIX re-delegated (High stops forward progress per gate rules).
- 2026-08-31 · US-15.1 Phase B RE-VALIDATE PASS WITH NOTES `4148bab`: both blockers independently re-verified closed (191/191 tests run directly, cron→live chain hand-traced, off-by-one fix hand-checked against CONTRACT constants); 0 new tsc errors. Non-blocking note: live cron response scoped to live-selected subset only (schema-required). → QA.
- 2026-08-31 · US-15.1 Phase B VALIDATE-FIX `36dc5da`: cron now routes allowlisted/capped eligible clients through `runWeeklyCycleLiveBatch` per rollout gates (new `runWeeklyCycleCronBatch`, one-line route.ts DI swap), others stay Phase A dry-run; 131 new tests across 22 suites covering all 8 CONTRACT-required categories (all passing); fixed a genuine off-by-one in outbox retry/backoff (first retry was 120s not frozen 30s; 4th attempt allowed past 3-attempt ceiling) found while writing tests; `approved_by_actor` audit stamp + `invokedBy:"system"` literal fixes. 0 new tsc errors (315 baseline, byte-identical). → re-run VALIDATE.
