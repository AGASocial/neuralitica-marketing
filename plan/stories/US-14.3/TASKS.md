...
- [ ] **Replay:** automated coverage — capture cookie values, run logout, replay the old `Cookie` on `getCurrentUser()` / a product GET → unauthenticated (login redirect / null user), not dashboard HTML.
