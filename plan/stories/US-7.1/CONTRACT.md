...
- [x] **Reviewed by FE** — `/operator/settings/cost-policy` form; `/operator/scripts` ConfirmDialog with `getReelBudgetPreview`; override UX; EN/ES `settings.costPolicy.*` + `scripts.budget.*`; batch blocked slot list.

**Reviewed by FE:** yes — 2026-08-30 — nextjs-frontend. **Settings:** new Server Component page at `/operator/settings/cost-policy` + client form (`getCostPolicyForSettings`, `updateGlobalCostPolicy`, `updateClientCostPolicyOverride`); PrimeReact inputs for `maxCostCents` (cents authority, currency display only) and `providerTier`; client override toggle section; read-only `resolvedLlmProviderLabel` + `effective` summary; optional `highTierWarningKey` banner; nav link in `AppHeader` operator block (placement not frozen in CONTRACT — FE adds alongside existing operator routes). **Scripts:** extend `ScriptsPageView` — call `getReelBudgetPreview` before all four generate/regenerate actions; PrimeReact `ConfirmDialog` (existing pattern from `AvatarConsentSection`); display server DTO fields + `projectionHintKey` → i18n; batch dialog lists `items` / `blockedSlotIndexes`; when `wouldExceed`, show override `InputTextarea` for `overrideReason` (1–500) + danger **Proceed anyway** → mutation with `budgetOverride: true` + reason; no client-side budget math. **Preview input:** pass `jobKind` `script_generate` \| `caption_generate` + `mode` batch/slot per CONTRACT; server returns resolved `jobKind` in preview DTO (fixture pattern for regenerate). **i18n:** `settings.costPolicy.*`, `scripts.budget.*` EN + ES. Types/constants from `lib/contracts/cost-policy.ts` (already aligned).
